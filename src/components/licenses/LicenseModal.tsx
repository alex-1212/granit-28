      ...
      <div ref={modalRef} className="relative bg-card rounded-xl max-w-3xl w-full max-h-[90vh] overflow-hidden animate-scale-in">
        <div className="absolute top-4 right-4 z-10">
          <button 
            ...
            aria-label={t('licenses.ui.modal.close')}
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-center p-4 h-full">
          <img 
            src={license.src} 
            alt={t(`licenses.ui.card.${license.id}.title`)} 
            className="max-w-full max-h-[80vh] object-contain" 
          />
        </div>
        
        <div className="p-4 border-t border-border">
          <h3 className="font-semibold mb-1 text-sm">{t(`licenses.ui.card.${license.id}.title`)}</h3>
