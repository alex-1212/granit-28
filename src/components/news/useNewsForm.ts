...
export function useNewsForm(initialData: NewsItem | undefined, onSuccess: () => void, onClose: () => void) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: isEditing ? t('news.ui.form.updatedToast') : t('news.ui.form.createdToast'),
          description: isEditing 
            ? t('news.ui.form.updatedDesc') 
            : t('news.ui.form.createdDesc'),
        });
        onSuccess();
        onClose();
      } else {
        toast({
          ...
          description: result.error || t('news.ui.form.saveErrorDesc'),
        });
      }
    } catch (error) {
