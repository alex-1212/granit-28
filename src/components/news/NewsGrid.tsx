...
const NewsGrid = ({ isLoading, news, filter, formatDate }: NewsGridProps) => {
  ...
  const { t } = useLanguage();
  const filterLabel = categoryLabelKeys[filter] ? t(categoryLabelKeys[filter]) : filter;
    ...
    return (
      <div className="text-center py-8 md:py-12">
        <p className="text-base md:text-lg text-muted-foreground">
          {t('news.ui.grid.emptyCategory').replace('{filter}', filterLabel)}
        </p>
      </div>
    );
  }
