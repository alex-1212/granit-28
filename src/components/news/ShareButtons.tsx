...
const ShareButtons = ({ title, url, description = '' }: ShareButtonsProps) => {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(url).then(() => {
      toast({
        title: t('news.ui.share.copiedTitle'),
        description: t('news.ui.share.copiedDesc'),
        duration: 3000,
      });
    }).catch(err => {
      ...
      toast({
        ...
        description: t('news.ui.share.copyFailed'),
        variant: "destructive",
        duration: 3000,
      });
    });
  ...
  return (
    <div className="flex flex-col gap-4 mt-6">
      <h3 className="text-lg font-medium">{t('news.ui.share.title')}</h3>
      <div className="flex flex-wrap gap-2">
        {shareLinks.map((item) => (
          <Button
            ...
            aria-label={t('news.ui.share.shareInAria').replace('{name}', item.name)}
          >
        ...
        <Button
          ...
          aria-label={t('news.ui.share.copyLink')}
        >
          <Link className="mr-2 h-4 w-4" />
          {t('news.ui.share.copyLink')}
        </Button>
      </div>
    </div>
  );
