    ...
    <div>
      ...
      <div className="glass-card rounded-xl overflow-hidden h-[400px] mb-6">
        <iframe 
          ...
          aria-label={t('contact.map.frameAriaLabel')}
        ></iframe>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <Button 
          variant="default" 
          className="flex-1 transition-all duration-300 active:scale-[0.98] hover:shadow-lg hover:-translate-y-0.5" 
          onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=${encodedAddress}`, '_blank', 'noopener,noreferrer')}
        >
          <Navigation className="mr-2" />
          {t('contact.map.route.google')}
        </Button>
        <Button 
          variant="default" 
          className="flex-1 transition-all duration-300 active:scale-[0.98] hover:shadow-lg hover:-translate-y-0.5" 
          onClick={() => window.open(`https://yandex.ru/maps/?rtext=~${encodedAddress}`, '_blank', 'noopener,noreferrer')}
        >
          <Navigation className="mr-2" />
          {t('contact.map.route.yandex')}
        </Button>
      </div>
    </div>
  );
