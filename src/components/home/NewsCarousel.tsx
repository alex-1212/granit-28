...
import { calculateReadingTime } from './utils/newsUtils';
...
const NewsCarousel = ({ formatDate: customFormatDate }: NewsCarouselProps) => {
  ...
  const localizedFormatDate = (dateString: string): string => {
    try {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-lg text-muted-foreground">
          {t('news.ui.carousel.empty')}
        </p>
      </div>
    );
  }

  const formatDateFn = customFormatDate || localizedFormatDate;
